...
suite('Inline script setup code action', () => {
    ...
    suite('provideCodeActions', () => {
        test('offers setup for an unresolved import in a not-yet-configured inline script', () => {
            ...
            assert.deepStrictEqual(actions[0].command?.arguments, [scriptUri, 'codeaction']);
        });
        ...
        test('offers setup again after the metadata block is edited to add the missing dependency', () => {
            ...
            // The user adds the module they were missing to the block; a changed metadata identity
            // resets the validated association, so the script needs setting up again.
        ...
        test('parses the in-memory buffer, so it works on an unsaved edit the CodeLens cannot see', () => {
            // Nothing has been saved, so the routing registry knows nothing about this file — the
            // state in which `provideCodeLenses` returns no lens.
