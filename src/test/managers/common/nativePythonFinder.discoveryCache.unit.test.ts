...
/**
 * Builds a fully-populated {@link ConfigurationOptions} fixture. Callers override individual fields
 * via the `overrides` argument to exercise a single differing field at a time.
 */
...
/** Minimal typed {@link NativeInfo} results tagged so different entries are distinguishable. */
...
suite('configurationEquals', () => {
    ...
    test('treats identical fsPath workspace directories as equal', () => {
        // fsPath-to-fsPath comparison: same underlying path built two ways stays equal.
...
suite('DiscoveryResultCache', () => {
    const ALL = 'all';
    const CONDA = 'Conda';
    // The concrete key format is irrelevant to the cache (it is key-format-agnostic); the real
    // getKey() joins URI arrays with '\0', but any distinct string works here.
    ...
    test('round-trips the exact configuration used to tag the entry', () => {
        // A hard-refresh result is tagged with its captured config; a lookup with a different config misses.
        const cache = new DiscoveryResultCache();
    ...
    test('a config change for one key does not invalidate other keys', () => {
        ...
        // The `all` scope is re-refreshed under a new configuration (configB).
        cache.set(ALL, configB, makeResults('all-2'), cache.generation);

        // The `Conda` scope keeps its own configA entry — updating `all` did not touch it.
    ...
    test('alternating all/kind/URI lookups with unrelated configs do not thrash', () => {
        ...
        // Repeatedly interleave lookups; each key retains its own valid entry (no eviction).
        for (let i = 0; i < 3; i++) {
    ...
    test('a store from a refresh that began before clear() does not repopulate', () => {
        const cache = new DiscoveryResultCache();
        const config = makeConfig();
        // A hard refresh captures the generation when it begins.
        const generationAtStart = cache.generation;

        // An explicit clear happens while that refresh is still in flight.
        cache.clear();

        // The in-flight refresh finishes and tries to store its (now stale) result.
    ...
    test('a store with the current generation after clear() succeeds', () => {
        ...
        const config = makeConfig();
        cache.clear();
        // A refresh started AFTER the clear captures the new generation and may store normally.
