...
suite('condaUtils.refreshCondaEnvs - failure vs. successful contract', () => {
    ...
    test('returns undefined when the native finder produces a non-array result (malformed discovery)', async () => {
        for (const bad of [null, undefined, { not: 'an array' }, 'oops']) {
            nativeFinder.refresh.resolves(bad as any);
            ...
            assert.strictEqual(result, undefined, `a non-array finder result (${String(bad)}) must be reported as failure`);
        }
    });
});
